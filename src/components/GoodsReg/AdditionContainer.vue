        ...
        <div class="addition_unit" v-for="(item, mainIndex) in additionOptions" :key="mainIndex">
            <template v-if="item.procTypeCode !== 4" >
              <dl class="addition_unit addition_dl">
                  <dt>추가구성명</dt>
                  <dd>
                      <input type="text" v-model="item.itemGroupName" class="text_input" maxlength="50"/>
                  </dd>
              </dl>
                      ...
                      <tr v-for="(subItem, index) in item.subAdditionOptions" :key="index">
                          <template v-if="subItem.procTypeCode !== 4" >
                            ...
                            <td style="vertical-align:middle">
                                <input type="checkbox" v-model="subItem.isSoldout"/>
                            </td>
...
export default {
  ...
  methods: {
    addAdditionOptionRow: function () {
      this.additionOptions.push({itemGroupName: '', subAdditionOptions: [{item: '', price: '', stockQty: '', isSoldout: false, isHide: false, procTypeCode: 2}], procTypeCode: 2})
    },
    ...
    addSubAdditionOptionRow: function (object, mainIndex) {
      this.additionOptions[mainIndex].subAdditionOptions.push({item: '', price: '', stockQty: '', isSoldout: false, isHide: false, procTypeCode: 2})
    },
