<template>
  <div id="contents">
                <h3>상품Q&amp;A관리</h3>
                <ul class="navi">
                    <li class="home"><a href="/" target="_top">홈</a></li>
                    <li>운영관리</li>
                    <li>고객운영관리</li>
                    <li class="on">상품Q&amp;A관리</li>
                </ul>
                <ul class="helpbox">
                    <li>상품의 Q&amp;A를 관리합니다.</li>
                </ul>



                <form name="Frm">
                    <div class="section_head">
                        <h4>오늘 등록된 Q&A는 총 <strong>0</strong>개입니다.</h4>
                        <div class="mgb5">
                            <select id="skey" name="skey" class="text_input">
                                <option value="">전체</option>
                                <option value="goods">상품명</option>
                                <option value="subject">제목</option>
                                <option value="content">내용</option>
                                <option value="answer">답변</option>
                                <option value="user">질문자</option>
                                <option value="dealer">답변자</option>
                            </select>

                            <input type="text" name="sword" maxlength="50" class="text_input" style="width:150px">
                            <button type="submit" class="btn btn-sm btn-default">검색</button>
                        </div>
                    </div>

                    <table class="t_list">
                        <caption>등록된 Q&A 리스트</caption>
                        <colgroup>
                            <col width="50">
                            <col width="*">
                            <col width="100">
                            <col width="100">
                        </colgroup>
                        <thead>
                            <tr>
                                <th>No</th>
                                <th>제목</th>
                                <th>고객명</th>
                                <th>등록일</th>
                            </tr>
                        </thead>
                        <tbody>

                            <tr style="background-color:#ffffe3">
                                <td>1</td>
                                <td class="left">
                                    상품 문의
                                    <font color="blue">(비밀글)</font>
                                </td>
                                <td><a href="javascript:;" onclick="ezpop.member.open(event)">wonej999</a></td>
                                <td>2019-05-03</td>
                            </tr>
                            <tr>
                                <td colspan="4" style="padding:10px">
                                    <div style="width:70px;text-align:left" class="fl"><img src="/img/201324253665762.jpg" width="50" height="50"></div>
                                    <div class="fl" style="margin:5px 0 0 0;width:80%;text-align:left"><a href="/goods/detail?gno=75" target="_blank">버든배낭</a></div>
                                </td>
                            </tr>
                            <tr style="background-color:#F8F8F8">
                                <td colspan="2" class="left">상품 문의 합니다.</td>
                                <td colspan="2">
                                    <!-- 수정/삭제/답글버튼 -->
                                    <button type="button" class="btn btn-sm btn-default" id="btnQna">수정</button>
                                    <button type="button" class="btn btn-sm btn-danger" onclick="delQuestion(39)">삭제</button>
                                    <button type="button" class="btn btn-sm btn-default" id="btnAnswer">답변</button>
                                </td>
                            </tr>
                        </tbody>
                    </table>

                    <div class="paging" style="margin-top:20px">
                        <span><a href="/management/qna_list.asp?page=1&skey=&sword="><strong>1</strong></a></span>
                    </div>
                </form>
  </div>
</template>

<script>
export default {

}
</script>

<style>

</style>